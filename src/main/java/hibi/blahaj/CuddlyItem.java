    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        if (stack.hasNbt()) {
            ...
            if (stack.hasCustomName()) {
                tooltip.add(new TranslatableText(TOOL_KEY_RENAME, getName(), Text.of(owner)).formatted(Formatting.GRAY));
            } else {
                tooltip.add(new TranslatableText(TOOL_KEY_OWNER, Text.of(owner)).formatted(Formatting.GRAY));
            }
        }
    }
